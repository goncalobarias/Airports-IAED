 ...
 * create a flight and allocates the necessary memory to store them. It
 * transforms some of the arguments into their appropriate form. Auxiliary
 * function to the 'v' command.
 */
flight *ReadFlight() {
    ...
    new_flight->flight_key =
        CreateFlightKey(new_flight->flight_code, new_flight->date_depart);
    ...
    new_flight->date_arrival =
        UpdateDate(new_flight->date_depart, new_flight->duration);
...
void AddFlight(global_store *global) {
    ...
    global->flightsTable =
        hashtable_insert(global->flightsTable, flight_node,
...
void ListAllFlights(global_store *global) {
    ...
    for (ptr = global->allFlights->first; ptr != NULL; ptr = ptr->next) {
        ...
        printf(FLIGHT_FULL_PRINT, flight_print->flight_code,
               flight_print->departure_id, flight_print->arrival_id);
...
int CheckAddFlightErrors(global_store *global, flight *new_flight) {
    flight *flight_check =
        GetFlight(global, new_flight->flight_code, new_flight->date_depart);
    ...
    if (CheckFlightCodeErrors(new_flight->flight_code)) {
        ...
        printf(AIRPORT_ERR_NO_ID,
               new_flight->arrival_id); /* no arrival id exists */
    } else if (CheckAirportExistence(global, new_flight->departure_id)) {
        printf(AIRPORT_ERR_NO_ID,
               new_flight->departure_id); /* no departure id exists */
...
char *CreateFlightKey(char *flight_code, clock *date_depart) {
    ...
    sprintf(flight_key, "%s %02d-%02d-%04d", flight_code, date_depart->day,
            date_depart->month, date_depart->year);
...
void PrintFlights(node_t *flight_head, const int mode) {
    ...
    for (ptr = flight_head; ptr != NULL; ptr = aux) {
        ...
        if (mode == DEPARTURE_INFO) {
            printf(FLIGHT_PRINT, tmp_flight->flight_code,
                   tmp_flight->arrival_id);
            PrintClock(tmp_flight->date_depart);
        } else {
            printf(FLIGHT_PRINT, tmp_flight->flight_code,
                   tmp_flight->departure_id);
