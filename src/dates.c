...
/* stores the accumulated days per month in a non leap year (jan = 1) */
const int days_months_ac[MONTHS] = {0,   31,  59,  90,  120, 151,
...
int ConvertDatesToMins(clock *formatted_date) {
    ...
    /* calculates the relative hour in minutes and adds the rest into relative
     * minutes */
 ...
 * one, 0 if both are equal dates or -1 if the first date is bigger then the
 * second.
 */
...
int CompareFlightDatesDeparture(void *flight_1, void *flight_2) {
    ...
    return CompareDates(flight_cmp_1->date_depart, flight_cmp_2->date_depart,
                        CMP_DATES_MIN);
}
...
int CompareFlightDatesArrival(void *flight_1, void *flight_2) {
    ...
    return CompareDates(flight_cmp_1->date_arrival, flight_cmp_2->date_arrival,
                        CMP_DATES_MIN);
}
