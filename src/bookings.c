...
void AddBooking(global_store *global, char *flight_code, clock *date_depart) {
    ...
    global->bookingsTable =
        hashtable_insert(global->bookingsTable, booking_node,
                         GetBookingKey(booking_node), GetBookingKey);
}
...
int CheckAddBookingErrors(global_store *global, char *flight_code,
    ...
    if (CheckBookingCodeErrors(new_booking->booking_code)) {
        ...
        printf(BOOKING_ERR_FLIGHT_CODE,
               flight_code); /* no parent flight exists */
        ...
        printf(BOOKING_ERR_PASSENGER); /* the passenger number is an invalid
                                          number */
    } else {
        return FALSE;
    }
 ...
 * booking codes are identical then it returns 0. Returns 1 if the second
 * booking code comes after in the lexicographic order,  0 if the bookings are
 * identical or -1 if the first booking code comes first in the lexicographic
 * order.
 */
...
void PrintBookings(node_t *booking_head) {
    ...
    for (ptr = booking_head; ptr != NULL; ptr = ptr->next) {
        ...
        printf(BOOKING_FULL_PRINT, tmp_booking->booking_code,
               tmp_booking->pas_num);
    }
}
...
void RemoveBooking(global_store *global, char *booking_code) {
    hash_elem *booking_elem = hashtable_get(global->bookingsTable, booking_code,
                                            booking_code, GetBookingKey);
